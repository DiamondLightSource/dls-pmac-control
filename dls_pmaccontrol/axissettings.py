...
class Axissettingsform(QDialog, Ui_formAxisSettings):
    ...
    def __init__(self, parent=None, currentMotor=1):
        ...
        self.lneCaptureFlag.setToolTip("""Capture n Flag Select Control
            ...
            3: User flag""")
        self.lneOutputMode.setToolTip("""Output n Mode Select (DLS: use 2 for 
        steppers)
...
""")
    ...
    def _updateAxisSignalControlsVars(self):
        (loopSelect, captureOn, captureFlag,
         outputMode) = self._getAxisSignalControlsVars()
    ...
    def _getAxisSignalControlsVars(self):
        ...
        if pmac.isMacroStationAxis(self.currentMotor):
            (loopSelect, captureOn, captureFlag,
             outputMode) = pmac.getAxisMsIVars(self.currentMotor,
                                               [910, 912, 913, 916])
        else:
            (loopSelect, captureOn, captureFlag,
             outputMode) = pmac.getOnboardAxisI7000PlusVars(self.currentMotor,
                                                            [0, 2, 3, 6])
    ...
    def sendIx11(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 11,
                                          self.lneIx11.text())
        self.axisUpdate()

    def sendIx12(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 12,
                                          self.lneIx12.text())
        self.axisUpdate()

    def sendIx13(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 13,
                                          self.lneIx13.text())
        self.axisUpdate()

    def sendIx14(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 14,
                                          self.lneIx14.text())
        self.axisUpdate()

    def sendIx15(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 15,
                                          self.lneIx15.text())
        self.axisUpdate()

    def sendIx16(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 16,
                                          self.lneIx16.text())
        self.axisUpdate()

    def sendIx17(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 17,
                                          self.lneIx17.text())
        self.axisUpdate()

    def sendIx19(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 19,
                                          self.lneIx19.text())
        self.axisUpdate()

    def sendIx20(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 20,
                                          self.lneIx20.text())
        self.axisUpdate()

    def sendIx21(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 21,
                                          self.lneIx21.text())
        self.axisUpdate()

    def sendIx22(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 22,
                                          self.lneIx22.text())
        self.axisUpdate()

    def sendIx23(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 23,
                                          self.lneIx23.text())
        self.axisUpdate()

    def sendIx24(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 24,
                                          self.lneIx24.text())
        self.axisUpdate()

    def sendIx25(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 25,
                                          self.lneIx25.text())
        self.axisUpdate()

    def sendIx26(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 26,
                                          self.lneIx26.text())
        self.axisUpdate()

    def sendIx30(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 30,
                                          self.lneIx30.text())
        self.axisUpdate()

    def sendIx31(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 31,
                                          self.lneIx31.text())
        self.axisUpdate()

    def sendIx32(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 32,
                                          self.lneIx32.text())
        self.axisUpdate()

    def sendIx33(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 33,
                                          self.lneIx33.text())
        self.axisUpdate()

    def sendIx34(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 34,
                                          self.lneIx34.text())
        self.axisUpdate()

    def sendIx35(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 35,
                                          self.lneIx35.text())
        self.axisUpdate()

    def sendIx65(self):
        self.parent.pmac.setAxisSetupIVar(self.currentMotor, 65,
                                          self.lneIx65.text())
        self.axisUpdate()
    ...
    def sendLoopSelect(self):
        ...
        if pmac.isMacroStationAxis(self.currentMotor):
            pmac.setAxisMsIVar(self.currentMotor, 910,
                               self.lneLoopSelect.text())
        else:
            pmac.setOnboardAxisI7000PlusIVar(self.currentMotor, 0,
                                             self.lneLoopSelect.text())
    ...
    def sendCaptureOn(self):
        ...
        if pmac.isMacroStationAxis(self.currentMotor):
            ...
            pmac.setOnboardAxisI7000PlusIVar(self.currentMotor, 2,
                                             self.lneCaptureOn.text())

    def sendCaptureFlag(self):
        pmac = self.parent.pmac
        if pmac.isMacroStationAxis(self.currentMotor):
            pmac.setAxisMsIVar(self.currentMotor, 913,
                               self.lneCaptureFlag.text())
        else:
            pmac.setOnboardAxisI7000PlusIVar(self.currentMotor, 3,
                                             self.lneCaptureFlag.text())

    def sendOutputMode(self):
        pmac = self.parent.pmac
        if pmac.isMacroStationAxis(self.currentMotor):
            pmac.setAxisMsIVar(self.currentMotor, 916,
                               self.lneOutputMode.text())
        else:
            pmac.setOnboardAxisI7000PlusIVar(self.currentMotor, 6,
                                             self.lneOutputMode.text())

...
# [STATE] This should only support displaying integer decimal i-variables at the moment, nothing more.

# [IDEA] Possibly use M-variable address definitions to check length of the variable
# [QUES] Is there any reasonable automated way of determining type and length of a variable? Ulrik says if a variable
#        is a "hex type" it will always return as a hex number, even though I wrote an dec int value to it. Or maybe it will
...
# [TODO] M variables may be up to 6 bytes (48 bits) long. I think this is only when they are floats -- but if this can happen
#        even for a non-float situation, then I should extend my bit-flip array to 48 bits long
...
# [TODO] Add continuous polling & warnings when value being edited has changed in the meantime
#        (could use row colouring just like in dls-dependency-checker)
...
def hexPmacFormattedToInt(s):
	assert type(s) == str
	isValid = re.match('^\$[0-9a-fA-F]+$', s) != None
	if not isValid:
		raise ValueError('String "%s" is not in PMAC hex format (use formatting as in "$4FA")')
	i = int(s[1:], 16) # use base 16 for the conversion to int
	return i

def int2bin(i):
	'''
	Oddly enough, I havent found a simple Python function or formatter to convert from integers to binary numbers. So this does the job.
	Can handle signed integers as many as 128 bits long.
	'''
	print 'int2bin: Would be nice if there were optional spacers between groups of 4 contiguous bits'
	if i == 0:
		return '0'
	if i < 0:
		sign = '-'
...
# Converts a string like '1200' or '$94fa' into int, using the appropriate radix; converts a string like '-0.234' into a float
...
class WatchesForm(formWatches):
	...
	def addWatch(self):
		...
		self.table.setText(noRows, 0, varName)  # set variable name column of the new row
...
class Watch:
	def __init__(self, pmac, varName):
		...
		self.pmac = pmac
##		self.isHex = None  # None here, but do set it in the contructor in child classes

		# Test that at least initially there is no problem reading variable value from the PMAC
	...
	def _sendPMACCommand(self, command):
		...
		if matchObject:
			raise ValueError('PMAC error code "%s" while retrieving "%s"' % (matchObject.group(1), self.varName))
	...
	def getVariableValue(self):
		'''
		Returns an int/long/float (Python types) depending on context. For hexadecimal variables, still returns an int with corresponing value.
		'''
	...
	def setVariableValue(self, newValue):
		...
		newValue may be either a str being a PMAC-formatted string or one of (int, long, float), in which case perform conversion as well.
		Throws exception on failure: this may be because newValue is not of appropriate Python type.
		'''
...
class MVariableWatch(Watch):
	...
	def __init__(self, pmac, varName):
		...
		# If called with Nonetype parameters, skip the normal initialization. For unit testing purposes only.
		if pmac == None and varName == None:
			return

		# Call parent class's contructor
		Watch.__init__(self, pmac, varName)

		# Parse address definition of the variable; this sets flags: self.signed, self.width, self.floatingPoint.
	...
	def _parseAddressDefinition(self, definitionStr):
		# Talk to the PMAC and figure out what the variable address definition is. we are interested in these booleans: 
		# * signed vs. unsigned
		# * width (no of bits)
		...
		# This is a special case of the next regex (read its comment first) but with an offset of 24,
		# which means "use all 24 bits and the offset is 0".
		matchObject = re.match('^([XY])\:?(\$?[0-9a-fA-F]+)\,24(\,([US]))?$', definitionStr)
		if matchObject:
			self.width = 24
			# the variable is signed when "S" is present, otherwise (either no letter, or a "U") the variable is unsigned
			self.signed = matchObject.group(4) != None and matchObject.group(4) == 'S'
			return matchObject

		# A "short" variable, up to 24 bits long, using only one of 2 memory banks (X and Y):
		#    Mxxx->bank(:)addr(,offset(,width(,format)))  -- here () means "optional"
		# The banks can be "X" or "Y" and accepted formats are "U" (unsigned, default) or "S" (signed, 2's complement)
		# The offset here can be 0..23, or a special value of 24 -- see next regex
		# The width can be from the set {1, 4, 8, 12, 16, 20, 24}
		matchObject = re.match('([XY])\:?(\$?[0-9a-fA-F]+)(\,(\d+)(\,(\d+)(\,([SU]))?)?)?', definitionStr)
		if matchObject:
			if matchObject.group(6):
				self.width = int(matchObject.group(6))
			# the variable is signed when "S" is present, otherwise (either no letter, or a "U") the variable is unsigned
			self.signed = matchObject.group(8) != None and matchObject.group(4) == 'S'
			return matchObject

		# Select a 48-bit floating-point variable (uses 32 of memory X and 16 bits of memory Y at that address)
		#    Mxxx->L(:)addr
		...
		# Select a 48-bit fixed-point signed (2's complement) variable (uses 32 of memory X and 16 bits of memory Y at that address)
		#    Mxxx->D(:)addr
		...
		# If the definition did not match any of the regexes, reset variable definition flags and raise an exception.
		# One typical case when this will happen is when the definition is simply "*".
		self.signed = None
		self.width = None
		self.floatingPoint = None
		raise ValueError('Address definition "%s" for M-variable "%s" is not supported' % (definition, self.varName))

	def setVariableValue(self, newValue):
		# accepts either a PMAC-formatted string, or int/long/float. however, checks whether the value is in range that is accepted by the PMAC.
		# note: PMAC alone does not complain when the new value's bit length is too long (ignores MSBs)
		pass
...
if __name__ == "__main__":
	...
	# Define unit tests (at least a few -- better than nothing). Call this function, on an open PMAC connection.
	...
	if connectionErrorMsg:
		...
		runTests(pmac_ts5)  # run some unit tests (not exhaustive at all) using the open interface to the PMAC

...
class GlobalStatusTest(unittest.TestCase):
    ...
    def test_inital_form(self, mock_control_form):
        obj = GlobalStatusForm(Controlform(options))
        obj.close()
    ...
    @patch("motor.Controlform")
    def test_ppmac_inital_form(self, mock_control_form):
        obj = PpmacGlobalStatusForm(Controlform(options))
        obj.close()

    @patch("motor.Controlform")
    def test_ppmac_update_status(self, mock_control_form):
        obj = PpmacGlobalStatusForm(Controlform(options))
